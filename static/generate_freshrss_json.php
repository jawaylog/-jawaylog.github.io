...
{
    $apiUrl = 'https://f.xieha.cn/p/api/greader.php';
            ...
            if (isset($subscriptions['subscriptions'])) {
                ...
                foreach ($articles['items'] as $article) {
                    ...
                    if (isset($subscriptionMap[$subscriptionId])) {
                        ...
                        $filename = 'https://f.xieha.cn/p/'.substr($iconUrl, strrpos($iconUrl, '/') + 1);
...
function saveToJsonFile($data)
{
    ...
    file_put_contents('freshrss.json', $json);
    echo '数据已保存到JSON文件中';
}

// 调用函数并提供用户名和密码
getAllSubscribedArticlesAndSaveToJson('welog', 'welog123');